...
namespace Lottery.Statistics.D11X5.D1
{
    ...
    /// 统计D1期数间隔的收敛。
    /// </summary>
    public class D1PeroidSpanLimit : BaseStatistics, IStatistics
    {
        public void StatAll()
        {
        ...
        private void Stat(object dbName)
        {
            ...
            DwPeroidSpanBiz biz = new DwPeroidSpanBiz(name, "D1");
            List<DwPeroidSpan> allNumbers = biz.GetAll("NumberId", "P", "Spans");
            string[] numberIds = this.GetAllD1Numbers().Select(x => x.ToString()).ToArray();

            string fileName = string.Format("Dw{0}_{1}.txt", name, "D1_PeroidSpanLimit");
            ...
            writer.Close();
            Console.WriteLine("{0},D1PeroidSpanLimit,Finished", dbName);
        }
    }
}
