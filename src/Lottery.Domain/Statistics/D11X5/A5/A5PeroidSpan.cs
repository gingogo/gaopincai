...
namespace Lottery.Statistics.D11X5.A5
{
    using Data;
    using Model.D11X5;
    using Model.Common;
    ...
    /// </summary>
    public class A5PeroidSpan : BaseStatistics, IStatistics
    {
        public void StatAll()
        {
        ...
        private void Stat(object dbName)
        {
            ...
            List<DwNumber> allNumbers = biz.DataAccessor.SelectWithCondition(string.Empty, "P", SortTypeEnum.ASC, null, "A5", "P", "N", "Date");
            for (int i = 0; i < allNumbers.Count; i++) allNumbers[i].Seq = i + 1;
            string[] allA5Numbers = NumberBiz.Instance.A5.Keys.ToArray();

            string fileName = string.Format("Dw{0}_{1}.txt", dbName.ToString(), "A5_PeroidSpan");
            string filePath = string.Format("{0}\\{1}", this._rootPath, fileName);
            StreamWriter writer = new StreamWriter(filePath, false, Encoding.UTF8);
            foreach (string numberId in allA5Numbers)
            {
                var numbers = allNumbers.Where(x => x.A5.Trim().Equals(numberId)).OrderBy(x => x.Seq).ToArray();
