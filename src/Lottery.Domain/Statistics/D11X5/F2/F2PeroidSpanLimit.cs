...
namespace Lottery.Statistics.D11X5.F2
{
    ...
    /// 统计F2期数间隔的收敛。
    /// </summary>
    public class F2PeroidSpanLimit : BaseStatistics, IStatistics
    {
        public void StatAll()
        {
        ...
        private void Stat(object dbName)
        {
            ...
            DwPeroidSpanBiz biz = new DwPeroidSpanBiz(name, "F2");
            List<DwPeroidSpan> allNumbers = biz.GetAll("NumberId","P", "Spans");
            HashSet<string> numberIds = this.GetAllF2Numbers(dbName.ToString());

            string fileName = string.Format("Dw{0}_{1}.txt", name, "F2_PeroidSpanLimit");
            ...
            writer.Close();
            Console.WriteLine("{0},F2PeroidSpanLimit,Finished", dbName);
        }
    }
}
