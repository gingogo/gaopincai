...
namespace Lottery.Statistics.D11X5.C3
{
    ...
    /// 统计C3期数间隔的收敛。
    /// </summary>
    public class C3PeroidSpanLimit : BaseStatistics, IStatistics
    {
        public void StatAll()
        {
        ...
        private void Stat(object dbName)
        {
            ...
            DwPeroidSpanBiz biz = new DwPeroidSpanBiz(name, "C3");
            List<DwPeroidSpan> allNumbers = biz.GetAll("NumberId", "P", "Spans");
            string[] numberIds = NumberBiz.Instance.C3.Keys.ToArray();

            string fileName = string.Format("Dw{0}_{1}.txt", name, "C3_PeroidSpanLimit");
            ...
            writer.Close();
            Console.WriteLine("{0},C3PeroidSpanLimit,Finished", dbName);
        }
    }
}
