...
namespace Lottery.Statistics.D11X5.C3
{
    using Data;
    using Model.D11X5;
    using Model.Common;
    ...
    /// </summary>
    public class C3PeroidSpan : BaseStatistics, IStatistics
    {
        public void StatAll()
        {
        ...
        private void Stat(object dbName)
        {
            ...
            List<DwNumber> allNumbers = biz.DataAccessor.SelectWithCondition(string.Empty, "P", SortTypeEnum.ASC, null, "C3", "P", "N", "Date");
            for (int i = 0; i < allNumbers.Count; i++) allNumbers[i].Seq = i + 1;
            string[] allC3Numbers = NumberBiz.Instance.C3.Keys.ToArray();

            string fileName = string.Format("Dw{0}_{1}.txt", dbName.ToString(), "C3_PeroidSpan");
            string filePath = string.Format("{0}\\{1}", this._rootPath, fileName);
            StreamWriter writer = new StreamWriter(filePath, false, Encoding.UTF8);
            foreach (string numberId in allC3Numbers)
            {
                var numbers = allNumbers.Where(x => x.C3.Trim().Equals(numberId)).OrderBy(x => x.Seq).ToArray();
