			...
			foreach (var category in categories)
			{
				...
				try
				{
					...
					if(category.Type.Equals("11X5"))
					{
						...
						D11X5.DwC5CXSpanBiz c5cxbiz = new D11X5.DwC5CXSpanBiz(category.DbName);
						foreach(string c5cx in c5cxs)
						{
							c5cxbiz.DataAccessor.TableName = ConfigHelper.GetDwSpanTableName(c5cx);
							peroidCount = c5cxbiz.DataAccessor.Count();
							maxSeq = c5cxbiz.DataAccessor.SelectMaxWithCondition("Seq",10,string.Empty);
							dict.Add(category.Name+"-"+c5cx,string.Format("{0}期,下载,Max Seq,{1}", peroidCount, maxSeq));
						}
					}
				}
				catch (Exception ex)
