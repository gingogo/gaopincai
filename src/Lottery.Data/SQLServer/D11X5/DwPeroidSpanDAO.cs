...
namespace Lottery.Data.SQLServer.D11X5
{
    ...
    /// <summary>
    /// DwPeroidSpanDAO提供表(PeroidSpan)的相关数据访问操作的类。
    /// </summary>
    public class DwPeroidSpanDAO
        : BaseDataAccess<DwPeroidSpan>
    {
        ...
        public DwPeroidSpanDAO(string tableName, string connectionString)
        ...
        protected override DwPeroidSpan DataReaderToEntity(SqlDataReader dr, MetaDataTable metaDataTable, params string[] columnNames)
        {
            if (dr == null)
                throw new ArgumentNullException("dr", "未将对象引用到实例");

            return EntityMapper.GetEntity(dr, new DwPeroidSpan(), this._tableName);
        }
        ...
        protected override DataFieldMapTable GetDataFieldMapTable(DwPeroidSpan dto, params string[] columnNames)
        {
            if (dto == null)
