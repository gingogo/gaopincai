...
namespace Lottery.Data.SQLServer
{
    /// <summary>
    ...
    public abstract class BaseSelect<T> : IBaseSelect<T> where T : BaseEntity
    {
        ...
        protected abstract T DataReaderToEntity(SqlDataReader dr,MetaDataTable metaDataTable,params string[] columnNames);
        ...
        public virtual bool IsExistWithCondition(string condition, params object[] parameterValues)
        {
            ...
            object result = SqlHelper.ExecuteScalar(this._connectionString, CommandType.Text, sqlCmd, parameters);
        ...
        public virtual PageData<T> SelectWithPageSizeByNotIn(int pageSize, int pageIndex, string condition, string notinColumnName, string orderByColumnName, SortTypeEnum sortType, object[] parameterValues, params string[] columnNames)
        {
            ...
            string sqlCmd = string.Format(sqlFormat.ToString(), columns, this._tableName,
                notinColumnName, tempTableSize, condition, param5, orderBy);
        ...
        /// <param name="columnNames">该实体对象中对应的数据库表的列名</param>
        /// <returns>实体对象集合</returns>
        protected virtual List<T> GetEntities(string sqlCmd, SqlParameter[] parameters, CommandType commandType, params string[] columnNames)
        {
        ...
        protected virtual SqlParameter[] FillParameters(object[] parameterValues)
        {
            ...
            SqlParameter[] parameters = new SqlParameter[parameterValues.Length];
        ...
        protected virtual void ExecuteByTransaction(List<SqlExpression> sqlExpressions, IsolationLevel isolationLevel)
        {
            ...
            {
                sqlConnection.Open();
                using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction(isolationLevel, "startPoint"))
                {
                    try
                    {
                        sqlExpressions.ForEach(expr =>
                            SqlHelper.ExecuteNonQuery(sqlTransaction, CommandType.Text, expr.CommandText, expr.Parameters));
