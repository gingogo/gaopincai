...
namespace Lottery.Data.SQLServer
{
    ...
    public abstract class BaseDataAccess<T> : BaseSelect<T>, IBaseDataAccess<T> where T : BaseEntity
    {
        ...
        protected SqlExpression GenerateInsertSqlExpression(DataFieldMapTable mapTable, string targetTable)
        {
            ...
            List<SqlParameter> sqlParameters = new List<SqlParameter>(mapTable.Count);
            foreach (string key in mapTable.Keys)
            {
                fields.AppendFormat("[{0}],", key);
                values.AppendFormat("@{0},", key);
                sqlParameters.Add(new SqlParameter("@" + key, mapTable[key]));
            }
        ...
        protected virtual SqlExpression GenerateUpdateSqlExpression(string condition, DataFieldMapTable mapTable, string targetTable, params object[] parameterValues)
        {
            ...
            {
                foreach (SqlParameter parameter in this.FillParameters(parameterValues))
                {
        ...
        protected virtual string GenerateUpdateSql(string condition, DataFieldMapTable mapTable, string targetTable, params object[] parameterValues)
        {
            ...
            if (parameterValues != null &&
                parameterValues.Length > 0)
            {
                sqlParameters = new List<SqlParameter>(parameterValues.Length);
                foreach (SqlParameter parameter in this.FillParameters(parameterValues))
                {
                    sqlParameters.Add(parameter);
                }
        ...
        public virtual int DeleteWithCondition(string condition, params object[] parameterValues)
        {
            ...
            return SqlHelper.ExecuteNonQuery(this._connectionString, CommandType.Text, sqlCmd, parameters);
        }
        ...
        /// <summary>
        /// 设置SqlBulkCopy中的ColumnMappings
        /// </summary>
        /// <param name="columnMappings">SqlBulkCopyColumnMappingCollection对象</param>
        /// <param name="metaDataTable">MetaDataTable对象</param>
        /// <param name="columnNames">目标表列名集合</param>
        private void SetColumnMappings(SqlBulkCopyColumnMappingCollection columnMappings, MetaDataTable metaDataTable, params string[] columnNames)
        {
            if (columnNames == null || columnNames.Length == 0)
            {
                foreach (var kp in metaDataTable.Columns)
