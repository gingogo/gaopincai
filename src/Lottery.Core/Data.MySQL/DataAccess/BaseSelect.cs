...
namespace Lottery.Data.MySQL
{
    ...
    public abstract class BaseSelect<T> : IBaseSelect<T> where T : BaseEntity
    {
        ...
        /// <param name="columnNames">该实体对象中对应的数据库表的列名</param>
        /// <returns>实体对象集合</returns>
        protected virtual List<TEntity> GetEntities<TEntity>(string sqlCmd, MySqlParameter[] parameters,
            Func<MySqlDataReader, MetaDataTable, string[], TEntity> drToEntityAction, params string[] columnNames)
        {
            return this.GetEntities(sqlCmd, parameters, CommandType.Text, drToEntityAction, columnNames);
        }
